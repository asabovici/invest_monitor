...
class _AgentSession:
    ...
    def ensure_agent(self):
        """Lazily instantiate the wrapped agent on first use.

        Anthropic client construction reads ``ANTHROPIC_API_KEY``; if it
        fails we surface the error so the API returns a 500 with a clear
...
def get_history(session_id: str) -> ChatHistory:
    ...
    raw = getattr(session._agent, "messages", [])
    turns: list[ChatTurn] = []
    for m in raw:
...
def _session_messages(session_id: str) -> tuple[str, str, list[dict]]:
    ...
    callers out of the private session shape.
    """
    session = _get_session(session_id)
    agent = session.ensure_agent()
